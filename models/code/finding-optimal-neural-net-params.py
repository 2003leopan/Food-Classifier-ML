...
def split_data(X, y, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    ...
    print(f"Test samples: {n_samples - n_train - n_val}")
...
def evaluate_hyperparameters(X, y, hyperparams_list, num_runs=10):
    """
    Evaluate different hyperparameter combinations by training multiple models
    and calculating average validation accuracies.
    
    Parameters:
        X - Input features
        y - Target labels
        hyperparams_list - List of dictionaries containing hyperparameter combinations
        num_runs - Number of models to train for each hyperparameter combination
    
    Returns:
        Dictionary containing average validation accuracies for each hyperparameter combination
    """
    results = {}
    ...
    for hyperparams in hyperparams_list:
        ...
        for run in range(num_runs):
            ...
            model = MLPModel(X.shape[1], 
                           num_hidden=hyperparams.get('hidden_size', 100),
            ...
            model.forward(X_val)
            predicted_labels = np.argmax(model.y, axis=1)
        ...
        results[str(hyperparams)] = {
            'average_accuracy': avg_accuracy,
            'std_accuracy': std_accuracy,
            'all_accuracies': accuracies
        }
        
        print(f"\nResults for {hyperparams}:")
...
def main():
    ...
    print(f"\nTesting {len(hyperparams_list)} hyperparameter combinations...")
    
    # Evaluate all combinations
    results = evaluate_hyperparameters(X, y, hyperparams_list)
    
    # Print results in a more organized way
    print("\nResults organized by parameter combinations:")
    print("------------------------------------------")
    
    # Group results by hidden size for better readability
    for hidden_size in hidden_sizes:
            ...
            for n_epochs in n_epochs_list:
                for batch_size in batch_sizes:
                    ...
                    if str(params) in results:
                        metrics = results[str(params)]
                        print(f"\nParameters: {params}")
                        print(f"Average validation accuracy: {metrics['average_accuracy']:.4f}")
                        print(f"Standard deviation: {metrics['std_accuracy']:.4f}")
    
    # Find best hyperparameters
    best_hyperparams = max(results.items(), key=lambda x: x[1]['average_accuracy'])
    print(f"\nBest overall hyperparameters: {best_hyperparams[0]}")
    print(f"Best average validation accuracy: {best_hyperparams[1]['average_accuracy']:.4f}")
    
    # Train final model with best hyperparameters on training + validation data
    ...
    final_model = MLPModel(X.shape[1],
                          num_hidden=best_hyperparams[1].get('hidden_size', 100),
                          num_classes=y.shape[1])
    
    train_sgd(final_model, X_train_val, y_train_val,
              alpha=best_hyperparams[1].get('alpha', 0.001),
              n_epochs=best_hyperparams[1].get('n_epochs', 100),
              batch_size=best_hyperparams[1].get('batch_size', 32),
              plot=True)
    
    # Evaluate on test set
    final_model.forward(X_test)
    predicted_labels = np.argmax(final_model.y, axis=1)
